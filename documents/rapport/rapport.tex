...
Pour rappel, une forme est connexe si elle est faite d'un seul morceau, une composante connexe étant donc un tel objet perçu sur l'image binaire.\\
				...
				\subsection{Fonctionnement technique}
					...
					\subsubsection{\textcolor{blue}{Fonctions d'étalonnage}}
						...
						Cette méthode est la première que nous avons utilisés, elle prend simplement la couleur du pixel au centre de l'objet suivi. Cette méthode fonctionne assez bien avec les objets ayants de grandes aires colorés avec peu de changement de saturation, ce qui n'est pas suffisant dans le cadre de notre projet.
						\paragraph{colorAverage} \paragraph{}
						colorAverage est plus élaborée que la méthode précedente. En effet, elle calcule la couleur moyenne dans une boîte englobante autour de l'objet, ceci permet donc d'avoir une couleur plus précise. Celle-ci fonction alors sur les objets unicolores mais est moins sensible au changement de valeur des couleurs de l'image. Par contre, cette fonction ne fonctionne absolument pas avec les objets multicolores, la moyenne de couleur n'ayant pas de sens sur ceux-ci.
						...
						Elle se déroule en deux étapes, la recherche de la couleur dominante, puis la moyenne des couleurs autour de celle-ci.
